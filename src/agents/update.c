...
agent_event_t update_agent(update_agent_config_t* cfg, update_agent_ctx_t* ctx, void** event_data) {
    ...
    ctx->err = fsm_init(&ctx->fsmc, &ctx->obj_t);
    if (ctx->err) {
    ...
    if (ctx->err) {
        ...
        PULL_RETURN(CALLBACK_ERROR, &ctx->err);
    }
    ...
    do {
        ...
        if (ctx->err) {
            ...
            PULL_RETURN(SUBSCRIBER_CHECK_ERROR, &ctx->err);
        }
    ...
    receiver_msg_t msg;
    ctx->err = fsm(&ctx->fsmc, &msg, sizeof(receiver_msg_t));
    if (ctx->err) {
    ...
    do {
        ctx->err = conn_request(ctx->conn, GET, "/firmware", &msg, sizeof(msg));
        if (ctx->err) {
            log_debug("Error sending the request\n");
            PULL_RETURN(SUBSCRIBER_CHECK_ERROR, &ctx->err);
        }
