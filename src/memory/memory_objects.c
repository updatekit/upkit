...
static pull_error swap_chunk(mem_object_t* src, mem_object_t dst, address_t offset, address_t swap_size, uint8_t* buffer, size_t buffer_size, bool cut_read, bool cut_write) {
    ...
    while (swap_full < swap_size) {
        ...
        done = memory_read(src, buffer, step, (cut_read)? offset%buffer_size: offset);
        if (done != step) {
            log_error(MEMORY_READ_ERROR, "Error reading from obj1\n");
            return MEMORY_READ_ERROR;
        }
        done = memory_write(dst, buffer, step, (cut_write)? offset%buffer_size: offset);
        if (done != step) {
...
pull_error swap_slots(mem_object_t* obj1, mem_object_t* obj2, mem_object_t* obj_swap,
    ...
    // of the buffer size
    PULL_ASSERT(swap_size % buffer_size == 0);
    mem_obj_t swap;
    ...
    while (offset < size) {
        ...
        while (swap_full < swap_size) {
            ...
            readed = memory_read(obj1, buffer, step, offset);
            if (readed != step) {
                log_error(MEMORY_READ_ERROR, "Error reading from obj1\n");
                return MEMORY_READ_ERROR;
            }
            written = memory_write(obj_swap, buffer, step, offset%buffer_size);
            if (written != step) {
                log_error(MEMORY_WRITE_ERROR, "Error writing the swap object\n");
                return MEMORY_WRITE_ERROR;
            }
        ...
        while (swap_full < swap_size) {
            ...
            readed = memory_read(obj2, buffer, step, offset);
            if (readed != step) {
                log_error(MEMORY_READ_ERROR, "Error reading from obj2\n");
                return MEMORY_READ_ERROR;
            }
            written = memory_write(obj1, buffer, step, offset);
            if (written != step) {
                log_error(MEMORY_WRITE_ERROR, "Error writing into obj1\n");
                return MEMORY_WRITE_ERROR;
            }
            swap_full += written;
            offset += step;
        }
        // Move obj_swap chunk into obj_2
        offset = offset - swap_size;
        step = buffer_size;
        swap_full = 0;
        while (swap_full < swap_size) {
            ...
            readed = memory_read(obj_swap, buffer, step, offset%buffer_size);
            if (readed != step) {
                log_error(MEMORY_READ_ERROR, "Error reading from obj_swap\n");
                return MEMORY_READ_ERROR;
            }
            written = memory_write(obj_1, buffer, step, offset);
            if (written != step) {
                 log_error(MEMORY_WRITE_ERROR, "Error writing into obj2\n");
                 return MEMORY_WRITE_ERROR;
            }
            swap_full += written;
            offset += step;
