...
// To invalidate the signature I just have to modify in some way the firmware
// This function randomly sends invalid blocks every time the block number is a
// multiple of 42, the answer to the ultimate question of life
extern void resource_firmware_invalid_signature(coap_context_t *ctx,
    ...
    if (block.num % 42 == 0) {
        // To invalidate the block I'm just sending it from a shifted position
        res = coap_add_block(response, server_ctx->mapped_file_len, server_ctx->mapped_file+42, block.num, block.szx);
    } else {
